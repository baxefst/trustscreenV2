...
export const useGaze = () => {
  ...
  useEffect(() => {
    const loadModels = async () => {
      ...
      const initModel = async (useGPU: boolean) => {
        try {
          const delegate = useGPU ? "GPU" : "CPU";
          const numFaces = useGPU ? 1 : 1; // Both are 1 as per request
          const numPoses = useGPU ? 1 : 1;
          ...
          console.warn(`Failed to initialize with ${useGPU ? "GPU" : "CPU"} delegate:`, err);
          return false;
        }
      };
      ...
      try {
        // Try GPU first
        const gpuSuccess = await initModel(true);
        if (!gpuSuccess) {
          // Fallback to CPU
          const cpuSuccess = await initModel(false);
          if (!cpuSuccess) {
  ...
  useEffect(() => {
    ...
    const startTracking = async () => {
      ...
      const track = async () => {
        ...
        // Performance optimization: Run detection every 2nd frame (approx 30fps)
        frameCount++;
        if (frameCount % 2 === 0) {
          ...
          if (poseResults.landmarks && poseResults.landmarks.length > 0) {
            ...
            if (leftWrist && rightWrist) {
              const leftVisible = Boolean(leftWrist.visibility && leftWrist.visibility > 0.3);
              const rightVisible = Boolean(rightWrist.visibility && rightWrist.visibility > 0.3);
        ...
        if (isTracking) {
          // Mobile throttling: Give UI thread a break
